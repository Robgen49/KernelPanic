...
import { fetchUpdateMyMember } from '../../api/updateMyMembers';
...
export const Profile = ({ allowEdit }: ProfileProps) => {
   ...
   const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
      ...
      if (team.teammates) {
         const oldMates = [...team.teammates];
         const newTeam = { ...team, teammates: oldMates.map(member => member.id === currentMember.id ? currentMember : member) };
         fetchUpdateMyTeam({ id: newTeam.id, teamName: newTeam.teamName, logo: newTeam.logo })
            .then((data) => data.json())
            .catch(() => {
               setErrorAlertOpen(true);
            })
            .then(() => {
               Promise.all(newTeam.teammates.map(el =>
                  fetchUpdateMyMember(el)
               )).then(() =>
         ...
         <Box height={'100vh'} display={'flex'} flexDirection={'column'}>
            ...
            <Container component="form" onSubmit={handleSubmit} sx={{ flexGrow: 1 }} >
               ...
               <Tabs
                  ...
                  onChange={(_: React.SyntheticEvent<Element, Event>, value: number) => {
                     if (team.teammates) {
                        const oldMates = [...team.teammates];
                        const newTeam = { ...team, teammates: oldMates.map(member => member.id === currentMember.id ? currentMember : member) };
                        setTeam(newTeam);
                        setCurrentMember(newTeam.teammates.find(member => member.id === value) as any);
                     }
                  }}
                  textColor="secondary"
